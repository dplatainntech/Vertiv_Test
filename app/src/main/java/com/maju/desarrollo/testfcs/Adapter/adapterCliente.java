...
public class adapterCliente extends BaseAdapter implements Filterable {
    ...
    Context contexto;
    List<searchCliente> listaObjetos,listaFiltrada;
    CustomFilter cs;

    public adapterCliente(Context contexto, List<searchCliente> listaObjetos) {
    ...
    public searchCliente obtenerItem(int position){
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        nomCliente.setText(listaObjetos.get(position).getCLiente());
    ...
    class CustomFilter extends Filter{
        ...
        protected FilterResults performFiltering(CharSequence constraint) {
            ...
            if(constraint!=null && constraint.length()>0){
                ...
                ArrayList<searchCliente> filtrados = new ArrayList<>();

                for(int i=0;i<listaFiltrada.size();i++){
                    if(listaFiltrada.get(i).getCLiente().toUpperCase().contains(constraint)){
        ...
        protected void publishResults(CharSequence constraint, FilterResults results) {
            listaObjetos= (ArrayList<searchCliente>)results.values;
